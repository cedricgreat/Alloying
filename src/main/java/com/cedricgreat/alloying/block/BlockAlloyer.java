...
import com.cedricgreat.alloying.network.MessageFluidTankContents;
import com.cedricgreat.alloying.tileentity.TileEntityAlloyer;
import com.cedricgreat.alloying.utility.CapabilityUtils;
...
public class BlockAlloyer<TE extends TileEntityAlloyer> extends BlockTileEntity<TE>
{

    public BlockAlloyer(final String blockName)
    {
        super(Material.IRON, blockName, true);
    ...
    public static List<ITextComponent> getFluidDataForDisplay(final IFluidTankProperties[] fluidTankProperties)
    {
        ...
        for (final IFluidTankProperties properties : fluidTankProperties)
        {
            ...
            {
                hasFluid = true;
                data.add(new TextComponentTranslation("tile.alloying:alloyer.fluid.desc", fluidStack.getLocalizedName(), fluidStack.amount, properties.getCapacity()));
            }
        }

        if(!hasFluid)
        {
            data.add(new TextComponentTranslation("tile.alloying:alloyer.empty.desc"));
        }
    ...
    public boolean onBlockActivated(final World worldIn, final BlockPos pos, final IBlockState state, final EntityPlayer playerIn, final EnumHand hand, final EnumFacing side, final float hitX, final float hitY, final float hitZ)
    {
        ...
        if(fluidHandler != null)
        {
            ...
            {
                Alloying.network.sendTo(new MessageFluidTankContents(fluidHandler.getTankProperties()), (EntityPlayerMP) playerIn);
            }
